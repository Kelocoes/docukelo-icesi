...
To get started, install the required development dependencies in your terminal:
...
Using `npm init jest@latest`, we can generate a `jest.config.ts` configuration file tailored to TypeScript and Express.

### 2.1 Interactive Jest Configuration

Running `npm init jest@latest` prompts configuration questions. Here are standard responses recommended for an Express TypeScript project:
...
The generated file is comprehensive; here is a streamlined initial configuration:
...
Jest setup also adds a `"test"` script to `package.json`:
...
Because we are using TypeScript, we must install `ts-jest` so Jest can compile TypeScript on the fly:

```bash
npm install --save-dev ts-jest
```

Finally, update `jest.config.ts` to use `ts-jest` as the preprocessor:
...
Here is a quick overview of each property configured in `jest.config.ts`:

- `clearMocks`: Clears mock calls and instances before each test.
- `collectCoverage`: Enables test coverage collection.
- `coverageDirectory`: Directory where coverage artifacts are written.
- `coverageProvider`: Coverage engine provider, set to `v8`.
- `preset`: Configures `ts-jest` as the TypeScript preprocessor.
- `testEnvironment`: Target runtime environment (`node`).
- `roots`: Root search paths for test files.
- `transform`: Instructs Jest to transform `.ts` files with `ts-jest`.
- `testRegex`: Regular expression matching test file names.
- `moduleFileExtensions`: Recognized module file extensions.


## 3. Folder Structure

To keep code organized, separate source code from automated tests. A standard layout places application code under `src` and tests under `tests` (or `src/tests`). Here is an example:

<img src="/img/computacion-3/jest-1.png" alt="Estructura de carpetas" width="500" />

## 4. Writing Your First Test

To verify the setup, let us create a simple utility in `src/utils`. Create `operations.util.ts`:
...
```
And export it inside `src/utils/index.ts`:

```typescript
export * from './operations.util';
```

Now create `src/tests/utils/operations.util.test.ts`:
...
Run test execution via terminal:
...
At this point, you might wonder why Jest initially reported 100% coverage with only one test. This occurs because Jest originally only measured files imported by the active test suite.

No obstante, quizá deseas comprobar la cobertura de tu código de una manera más detallada y ver qué líneas de código están cubiertas por las pruebas. To track overall repository coverage, configure `collectCoverageFrom` in `jest.config.ts`: y especificar qué archivos deseas incluir en la recolección de cobertura. Aquí tienes un ejemplo de cómo podrías configurarlo:
...
const config: Config = {
    ...
    collectCoverageFrom: [
        ...
        "!src/index.ts", // Exclude main entrypoint
        "!src/interfaces/**/*.ts", // Exclude interfaces
        "!src/config/**/*.ts", // Exclude configuration files
        "!src/models/**/*.ts", // Exclude models
        "!src/routes/**/*.ts", // Exclude routes
        "!src/**/index.ts", // Exclude barrel index exports
        "!src/validators/**/*.ts", // Exclude validation schemas
    ],
};
```
Now re-run `npm test`. You will see a detailed coverage breakdown:, indicando qué líneas de código han sido cubiertas por las pruebas y cuáles no.
...
#### 5.2.3 Mocking External Dependencies
...
#### 5.2.5 First Test: Successful User Creation
...
#### 5.2.6 Second Test: Error When User Already Exists
...
#### 5.3.3 Mocking the User Service
...
#### 5.3.4 Setting up Mock Request and Response Objects
...
#### 5.3.5 First Test: Successful User Creation
...
#### 5.3.6 Second Test: Error Handling
...
#### 5.3.9 Testing Update and Delete Operations
...
#### 5.3.10 Complete Controller Test File
...
## 6. Coverage Results After Implementing Tests
